...
enum {NVOCMP_FINDANY = 0, NVOCMP_FINDSYSID, NVOCMP_FINDITMID,
    NVOCMP_FINDSTRICT};
...
static int8_t     NVOCMP_findItem(NVOCMP_nvHandle_t *pNvHandle, uint8_t pg, uint16_t ofs,
                                  NVOCMP_itemHdr_t *pHdr, uint8_t flag);
static uint8_t    NVOCMP_newItem(NVOCMP_nvHandle_t *pNvHandle, NVOCMP_itemHdr_t *iHdr,
                                 uint8_t *pBuf);
...
static void       NVOCMP_readHeader(uint8_t pg, uint16_t ofs, NVOCMP_itemHdr_t *iHdr);
...
static uint8_t    NVOCMP_doNVCRC(uint8_t pg, uint16_t ofs, uint16_t len, uint8_t crc);
static uint8_t    NVOCMP_doRAMCRC(uint8_t *input, uint16_t len, uint8_t crc);
static uint8_t    NVOCMP_verifyCRC(uint16_t iOfs, uint16_t len, uint8_t crc, uint8_t pg);
...
static uint8_t NVOCMP_createItemApi(NVINTF_itemID_t id, uint32_t len, void *pBuf)
{
    ...
    if (NVOCMP_findItem(&NVOCMP_nvHandle, NVOCMP_nvHandle.actPage, NVOCMP_nvHandle.actOffset, &iHdr, NVOCMP_FINDSTRICT) == NVINTF_NOTFOUND)
    {
        // Create the new item
        err = NVOCMP_newItem(&NVOCMP_nvHandle, &iHdr, pBuf);
    }
    else
    {
      // Item already exists
...
static uint8_t NVOCMP_deleteItemApi(NVINTF_itemID_t id)
{
    ...
    if(!err)
    {
      ...
      err = (NVOCMP_findItem(&NVOCMP_nvHandle, NVOCMP_nvHandle.actPage, NVOCMP_nvHandle.actOffset,
                             &iHdr, NVOCMP_FINDSTRICT) == NVINTF_NOTFOUND) ?
...
static uint32_t NVOCMP_getItemLenApi(NVINTF_itemID_t id)
{
    ...
    len = (NVOCMP_findItem(&NVOCMP_nvHandle, NVOCMP_nvHandle.actPage, NVOCMP_nvHandle.actOffset, &iHdr, NVOCMP_FINDSTRICT) != NVINTF_SUCCESS) ? 0 : iHdr.len;
                                  ...
                                  void *pBuf)
{
    ...
    if (!err)
    {
        err = NVOCMP_readItem(&iHdr, ofs, len, pBuf);
    }
...
static uint8_t NVOCMP_doNextApi(NVINTF_nvProxy_t * prx)
{
    ...
    // Look for item
    if (!NVOCMP_findItem(&NVOCMP_nvHandle, sPage, fOfs, &hdr, search))
    {
        iOfs = hdr.hofs;
        ...
        case doRead:
            ...
            {
                status = NVOCMP_readItem(&hdr, 0, hdr.len, prx->buffer);
            }
            break;
        case doDelete:
...
static void NVOCMP_migratePage(NVOCMP_nvHandle_t *pNvHandle, uint8_t page)
{
  ...
  if(offset1 - NVOCTP_PGDATAOFS > FLASH_PAGE_SIZE - NVOCMP_PGDATAOFS)
  {
    ...
    {
      NVOCMP_findItem(pNvHandle, page, offset2, &iHdr, NVOCMP_FINDANY);
      offset2 = iHdr.hofs - iHdr.len;
    }
  }

  dstPg = NVOCMP_INCPAGE(page);
  ...
  if((pNvHandle->nvSize > 2) && (offset2 - NVOCTP_PGDATAOFS > 0))
  {
                               ...
                               NVOCMP_pageInfo_t *pPageInfo)
{
  ...
  if(thisHdr.pageOffset != NVOCMP_NULLOFFSET)
  {
                              ...
                              uint8_t *pBuf)
{
    ...
    if(NVOCMP_failW == NVINTF_SUCCESS)
    {
      ...
      (void)NVOCMP_findItem(pNvHandle, pNvHandle->actPage, pNvHandle->actOffset, &hdr, NVOCMP_FINDSTRICT);
 ...
 * @return  none
 */
static void NVOCMP_readHeader(uint8_t pg, uint16_t ofs, NVOCMP_itemHdr_t *pHdr)
{
#if 0
                               ...
                               void *pBuf)
{
    ...
    if (NVOCMP_CRCONREAD)
    {
        err = NVOCMP_verifyCRC(iOfs, iHdr->len, iHdr->crc8, iHdr->hpage);
    }
    if(err == NVINTF_SUCCESS)
    {
...
static NVOCMP_compactStatus_t NVOCMP_compact(NVOCMP_nvHandle_t *pNvHandle)
{
    ...
    while(srcPg != dstPg)
    {
        ...
        if(NVOCMP_failW == NVINTF_SUCCESS)
        {
            ...
            // Read and decompress item header
            NVOCMP_readHeader(srcPg, srcOff, &srcHdr);
            dataLen  = srcHdr.len;
            ...
            else
            {
              if(!(srcHdr.stats & NVOCMP_VALIDIDBIT) && srcHdr.stats & NVOCMP_ACTIVEIDBIT)
              {
                ...
                if(NVOCMP_verifyCRC(crcOff,dataLen,srcHdr.crc8, srcPg))
                {
...
static uint8_t NVOCMP_verifyCRC(uint16_t iOfs, uint16_t len, uint8_t crc, uint8_t pg)
{
    ...
    newCRC = NVOCMP_doNVCRC(pg, iOfs, crcLen, 0);
